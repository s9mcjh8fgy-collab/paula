...
name: carrossel
description: >
  Cria carrosséis completos para Instagram e TikTok com a identidade visual da Paula Corrêa Advocacia.
  Parte de casos reais e anonimizados do escritório (linha editorial "Advocacia através dos meus casos"),
  com foco no nicho de Advocacia da Construção Civil (arquitetos, engenheiros, construtoras,
  imobiliárias, administradoras de condomínio). Gera texto editorial, HTMLs estilizados,
  renderiza em PNG via Playwright.
  Use quando o usuário mencionar "carrossel", "carousel", "slides instagram",
  "slides tiktok", "faz um carrossel", ou pedir pra transformar um caso/tema em carrossel.
---
...
Todo carrossel deve, sempre que possível, partir de um caso ou situação real do escritório — sem
identificar o cliente (trocar nome, cidade e detalhes que permitam reconhecimento). Se o usuário não
trouxer um caso, checar `conteudo/casos/` por ideias registradas antes de perguntar. O ângulo por
padrão é voltado ao nicho de Advocacia da Construção Civil, mas casos de outras áreas (família,
trabalhista, consumidor) também podem virar conteúdo quando fizer sentido pro negócio.
...
5. **Briefing rápido** — perguntar ao usuário (numa mensagem só):
   ...
   >   - Se tiver imagens: joga na pasta `conteudo/instagram/carrossel/[tema]/imagens/` e me diz os nomes
   ...
   Se o usuário fornecer imagens, criar a pasta `conteudo/instagram/carrossel/[tema]/imagens/` e confirmar que as imagens estão lá antes de seguir. Se for gerar a versão TikTok também (Fase 3), copiar as mesmas imagens pra `conteudo/tiktok/carrossel/[tema]/imagens/`.
...
10. Salvar tudo em `conteudo/instagram/carrossel/[tema]/carousel-text.md` (se a versão TikTok for gerada na Fase 3, copiar o mesmo arquivo pra `conteudo/tiktok/carrossel/[tema]/carousel-text.md`)
...
4. Salvar HTMLs em `conteudo/instagram/carrossel/[tema]/`
...
Salvar PNGs em `conteudo/instagram/carrossel/[tema]/`.
...
- Renderizar via CLI:
  ...
  ```
- Salvar em `conteudo/tiktok/carrossel/[tema]/` (copiar também `carousel-text.md` e a pasta `imagens/`, se houver, pra deixar a pasta autossuficiente)

---

### Fase 4 — Stories (opcional)

Depois de finalizar o Instagram (Fase 2) ou a versão TikTok (Fase 3), perguntar:
> "Quer gerar também os stories de apoio? (teaser do post + banco de respostas pra caixinha de pergunta)"

Se sim, gerar dois artefatos:

**1. Story teaser** — imagem vertical (1080x1920) anunciando o post do feed, pra postar no story no mesmo dia.
- Reaproveita o ângulo/capa do carrossel (título curto, sem repetir o slide 1 igual)
- CTA de arrastar pro post: "olha o carrossel completo" ou similar, nunca CTA de contato direto (ver regra de CTA abaixo)
- Fotos reais da Paula (`marca/fotos/`, ver `marca/design-guide.md`) podem entrar aqui como fundo ou elemento — story é o formato mais natural pra isso, mais pessoal que o carrossel
- **Safe zones do Instagram** (não colocar texto/elementos importantes nessas áreas, ficam cobertas pela UI nativa):
  - Topo: ~250px (foto de perfil, nome, botão fechar)
  - Base: ~250px (caixa de resposta/reply)
- Renderizar via CLI: `npx playwright screenshot --viewport-size=1080,1920 --full-page "file:///caminho/absoluto/story-teaser.html" "story-teaser.png"`

**2. Story caixinha de pergunta** — segunda imagem, também 1080x1920, com o convite pra pergunta:
- Texto tipo "me pergunta o que quiser sobre [tema do post]" ou "dúvida sobre [tema]? pergunta aqui"
- Deixar uma área central livre e sinalizada (ex: comentário no HTML) pra Paula posicionar o sticker nativo de pergunta do Instagram — a arte não desenha o sticker, só reserva o espaço, já que o sticker é aplicado dentro do app
- Mesma regra de safe zone acima

**3. Banco de respostas** — documento markdown com perguntas antecipadas + respostas prontas, baseado no tema do dia, pra Paula colar rápido quando a pergunta real chegar na caixinha (a caixinha é aberta de verdade pelos seguidores; isso não é resposta automática nem pergunta fabricada, é só preparo prévio):
- Listar 5-8 perguntas prováveis sobre o tema (dúvidas comuns do nicho, variações do que já apareceu em `conteudo/casos/` ou nas demandas consultivas)
- Pra cada uma, uma resposta curta (1-3 frases, tom de story: mais solto que o carrossel, como se estivesse respondendo no directo)
- Reforçar: nunca CTA de contato direto tipo "me chama" ou "manda mensagem" nas respostas — se a dúvida for específica demais pro caso da pessoa, fechar com algo como "isso já depende do caso, mas me conta mais nos comentários" em vez de pedir contato privado
- Salvar em `conteudo/instagram/carrossel/[tema]/stories/banco-respostas.md`

**Regra de CTA (vale pra qualquer story):** nunca usar "me chama", "me procure", "manda mensagem" — usar CTA de engajamento ("comenta", "manda pra quem precisa ler isso", "responde aqui"). Isso vale pelo Código de Ética da OAB (restrição a captação de clientela).

**Regra de honestidade:** a caixinha de pergunta é sempre aberta de verdade pros seguidores responderem. Nunca fabricar perguntas e postar como se fossem de um seguidor real — isso é forjar engajamento e pode configurar publicidade enganosa.

Salvar tudo em `conteudo/instagram/carrossel/[tema]/stories/` (ver estrutura no Output final).

---

## Output final

Estrutura por rede social, com uma pasta autossuficiente por postagem (evita bagunça quando acumular vários carrosséis):
  ...
  instagram/
    carrossel/
      [tema]/
        ...
        ...
        stories/ (se gerado na Fase 4)
          story-teaser.html -> story-teaser.png
          story-pergunta.html -> story-pergunta.png
          banco-respostas.md
...
2. Gerar com Pollinations, salvar em `conteudo/instagram/carrossel/[tema]/imagens/`
