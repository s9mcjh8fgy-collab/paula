...
export default async function SearchPage({
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-lg font-semibold text-gray-900">Buscar atendimentos</h1>
              ...
              {results.map((i) => (
                <li key={i.id} className="px-4 py-3">
                  <div className="mb-1 flex flex-wrap items-center gap-2 text-xs text-gray-500">
                    ...
                    <span>·</span>
                    <Link href={`/clients/${i.client_id}`} className="font-medium text-blue-600 hover:underline">
                      {i.clients?.name}
                    </Link>
                    <span>·</span>
                  ...
                  {i.response && <p className="mt-1 text-sm text-gray-700">{i.response}</p>}
                </li>
              ))}
            </ul>
          )}
